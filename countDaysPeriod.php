...
//разбивка начальной конечной даты на составляющие (год, месяц, дата)
	...
	$dm_g = $d_s[1];
	$d_e = explode("-",$end_d);
//блок подсчета количество дней между двумя датами включая начальную и конечную дату
	$d1_ts = strtotime($start_d);
	$d2_ts = strtotime($end_d);
	$seconds = abs($d1_ts - $d2_ts);
	$c_days = floor($seconds / 86400)+1;
